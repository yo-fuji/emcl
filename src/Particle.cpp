...
double Particle::likelihood(LikelihoodFieldMap* map, Scan& scan)
{
  ...
  double lidar_x = p_.x_ + scan.lidar_pose_x_ * cos_[t] - scan.lidar_pose_y_ * sin_[t];
  double lidar_y = p_.y_ + scan.lidar_pose_x_ * sin_[t] + scan.lidar_pose_y_ * cos_[t];
  ...
  for (size_t i = 0; i < scan.ranges_.size(); i += scan.scan_increment_) {
    ...
    double lx = lidar_x + scan.ranges_[i] * cos_[a];
    double ly = lidar_y + scan.ranges_[i] * sin_[a];
                             ...
                             LikelihoodFieldMap* map, double& hit_lx, double& hit_ly)
{
  ...
  for (double d = map->resolution_; d < range; d += map->resolution_) {
    double lx = ox + d * cos_[direction];
    double ly = oy + d * sin_[direction];
...
{
  double p1_x = ox + range1 * cos_[direction1];
  double p1_y = oy + range1 * sin_[direction1];
  double p2_x = ox + range2 * cos_[direction2];
  double p2_y = oy + range2 * sin_[direction2];
