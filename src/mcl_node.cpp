...
#include "emcl/mcl_node.hpp"
#include "emcl/Pose.hpp"

#include "rclcpp_components/register_node_macro.hpp"
RCLCPP_COMPONENTS_REGISTER_NODE(emcl::MclNode)
...
MclNode::MclNode(const rclcpp::NodeOptions& options)
  : LifecycleNode("mcl_node", "", options)
{
...
MclNode::on_configure(const rclcpp_lifecycle::State& state)
{
  (void)state;
...
nav2_util::CallbackReturn
MclNode::on_activate(const rclcpp_lifecycle::State& state)
{
  (void)state;
...
nav2_util::CallbackReturn
MclNode::on_deactivate(const rclcpp_lifecycle::State& state)
{
  (void)state;
...
nav2_util::CallbackReturn
MclNode::on_cleanup(const rclcpp_lifecycle::State& state)
{
  (void)state;
...
nav2_util::CallbackReturn
MclNode::on_shutdown(const rclcpp_lifecycle::State& state)
{
  (void)state;
...
void MclNode::initCommunication()
{
  ...
  laser_scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
    ...
    std::bind(&MclNode::cbScan, this, _1));
  initial_pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
    "initialpose", rclcpp::SystemDefaultsQoS(),
    std::bind(&MclNode::initialPoseReceived, this, _1));
  map_sub_ = this->create_subscription<nav_msgs::msg::OccupancyGrid>(
    "map", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable(),
    std::bind(&MclNode::mapReceived, this, _1));

  global_loc_srv_ = this->create_service<std_srvs::srv::Empty>(
    "global_localization", std::bind(&MclNode::cbSimpleReset, this, _1, _2, _3));
...
std::shared_ptr<OdomModel> MclNode::initOdometry()
{
  double ff, fr, rf, rr;
...
void MclNode::cbScan(const sensor_msgs::msg::LaserScan::ConstSharedPtr& msg)
{
...
void MclNode::initialPoseReceived(const geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr& msg)
{
  init_request_ = true;
...
void MclNode::mapReceived(const nav_msgs::msg::OccupancyGrid::ConstSharedPtr& msg)
{
...
void MclNode::publishPose(const rclcpp::Time& stamp,
...
void MclNode::publishOdomFrame(const rclcpp::Time& stamp,
                               double x, double y, double t)
{
...
void MclNode::publishParticles(const rclcpp::Time& stamp)
{
...
bool MclNode::cbSimpleReset(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
